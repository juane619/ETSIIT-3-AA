...
# El objetivo es predecir el nivel de sonido en decibelios en función de los atributos dados (1-5)

datos = read.csv("datos/airfoil_self_noise.csv", header = FALSE)
names(datos)[1] = "Frequency"
names(datos)[2] = "Angle of attack"
names(datos)[3] = "Chord length"
names(datos)[4] = "Free stream velocity"
names(datos)[5] = "Displacement thickness"
names(datos)[6] = "Sound level"

library(caret)

train.index = createDataPartition(datos$`Sound level`, p = 0.7, list = FALSE)   # Igual que StratifiedKFolds de SKlearn, mantiene la proporción de valores (iguales)
train = datos[train.index,]
test = datos[-train.index,]

features_train = data.matrix(subset(train, select = -6))
labels_train   = data.matrix(subset(train, select = 6))
features_test  = data.matrix(subset(test,  select = -6))
labels_test    = data.matrix(subset(test,  select = 6))
...
# En el plot se pueden ver las características y cuales de ellas son separables o por el contrario no

# http://www.datasciencedude.com/2015/05/31/test/

# Datos listos para ser procesados por el modelo
