...
Una vez conocido nuestro problema y preprocesado los datos, nuestro modelo sería el siguiente:
...
Como vemos, tenemos sobreajuste (error dentro de la muestra 0), por lo que hace falta regularización.
...
Como vemos, los errores nos muestran que nuestro modelo regularizado con ridge se comporta peor que regularizado con lasso, es porque lasso directamente elimina variables (características) y ridge tiene algo mas de ruido, es decir, nos quedamos con el **modelo de regresión logística con regularización Lasso**.
...
#Como se ve en la grafica, las variables "disp_thick" y "angle attack" son dependientes unas de otra, por tanto, hay que quitar una de ellas. Decidimos eliminar "displacement thickness".
datos=datos[,-5]
```
...
#createFolds (de la misma familia que esta) divide el dataset en k FOLDS, valido para validacion cruzada.
...
plot(cv$glmnet.fit, "norm", ylim = c(-1,0.6), col = c(1:length(features_train)))
abline(h = best_lambda, col = "orange")
sprintf("Mejor lambda en las %s particiones es = %s", kfolds, best_lambda)
```


### 5. Regularización, modelo a usar e hyperparámetros.

En un principio, la regularización en este caso no parece necesaria ya que tenemos pocas características. Aplicaremos regularización para comparar un modelo con otro. Como ya sabemos la función de regularización depende de un término lambda, dicho término (hiperparámetro) lo estimaremos usando validación cruzada.

Procedemos a ajustar el modelo usando regresión normal sin regularización, con la variable "displacement thickness" ya eliminada.
...
# (CV) Una vez tenemos el mejor lambda, procedemos a ajustar nuestro modelo:
nparticiones = 5

folds = createFolds(train, nparticiones)
```
...
for (i in 1:nparticiones) {
    cv_reg = folds[i](sound_level ~ ., data=train, method="lm")
    ein = ein + eval_model(ans_reg)[1]
    eout = eout + eval_model(ans_reg)[2]
    
    sprintf("Error dentro del train, particion %s es %s", i, eval_model(ans_reg)[1])
    sprintf("Error dentro del test, particion %s es %s", i, eval_model(ans_reg)[2])
}
